...
const AuctionCardHeader = React.memo(({ status, timeLeft, isTimeUrgent }: AuctionCardHeaderProps) => {
  ...
  const getTimeDisplay = () => {
    return (status !== "in-progress" && status !== "winning" && status !== "outbid")
      ? "Ended"
      : timeLeft
  }
...
const AuctionCardImage = React.memo(({ imageUrl, title }: AuctionCardImageProps) => {
...
const AuctionCardActions = React.memo(({ onDelete, onEdit }: AuctionCardActionsProps) => {
...
const MemoizedAuctionCard = React.memo(AuctionCard, (prevProps, nextProps) => {
  const criticalProps: (keyof AuctionCardProps)[] = [
    'title', 'price', 'status', 'timeLeft', 'isTimeUrgent',
    'endTime', 'imageUrl', 'variant', 'className', 'onClick'
  ]
